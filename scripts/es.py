...
def tickets_orig(mongo, es, type, start, end, chunk):
    ...
    if mongo and es and type and start and end and chunk:
        try:
            ...
            i = -1                  # If document not exist, it occurs 'UnboundLocalError'
            for i, (ok, result) in enumerate(
                    streaming_bulk(client=es_client, actions=add_tickets_index(cur), index='tickets',
...
def add_tickets_index(cursor):
    for c in cursor:
        yield gen_ticket(c)


def gen_ticket(cursor):
    ticket = dict(
        _id=str(cursor['_id']),
        enabled=cursor['enabled'],
    ...
    if 'user_oid' in cursor:
        ticket['user_oid'] = str(cursor['user_oid'])
...
cli = click.CommandCollection(sources=[invitation, ticket, user])
